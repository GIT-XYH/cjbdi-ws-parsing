 ...
 * @Description: 解析文本, 获取相应字段, 写入 kafka
 */
...
public class AnalysisProcessFunction extends ProcessFunction<WsBeanWithFile, String> {
    ...
    public void processElement(WsBeanWithFile wsBeanFromKafka, Context context, Collector<String> collector) throws Exception {
        ...
        try {
            ...
            base64File = wsBeanFromKafka.getBase64File();
            //wsDoc 就是二进制格式的文书原文件
...
//            FileUtils.writeByteArrayToFile(new File("/Users/xuyuanhang/Desktop/code/cjbdi-ws-parsing/data/wsDoc"), wsDoc);
        ...
        String text = null;
        if (!c_wsText.equals("")){
            text = HtmlParser.getPlainText(c_wsText);
        }


        /**
         * 将 text 转成 json 并提取其中所需要的子 jsonObject 变量定义
         */
...
//            post = sendPost(jobParameters.getRequired("post-url"), fullText);
        ...
        /**
         * 对于解析结果和 bean 字段名称名称不一样的字段重新赋值
         */
        if (esDataBean != null) {
            esDataBean.court = jo2.getString("courtname");
        }
        String judgedate = jo2.getString("judgedate");
        if (esDataBean.court == null) {
            log.warn("court is null");
        }
        esDataBean.judgedb_plaintiff_number = jo2.getString("plaintiffnum");
        if (esDataBean.judgedb_plaintiff_number == null) {
            log.warn("judgedb_plaintiff_number is null");
        }
        esDataBean.applicable_law = jo2.getString("applicablelaws");
        if (esDataBean.applicable_law == null) {
            log.warn("applicable_law is null");
        }
        esDataBean.related_law = jo2.getString("relatedlows");
        if (esDataBean.related_law == null) {
            log.warn("related_law is null");
        }
        esDataBean.pcourt = jo2.getString("parentcourtname");
        if (esDataBean.pcourt == null) {
            log.warn("pcourt is null");
        }
        if (judgedate != null && judgedate.length()>=4) {
            String year = judgedate.substring(0, 4);
            esDataBean.judgeyear = Integer.valueOf(year);
        } else {
            log.error("judgedate is null");
        }


        /**
         * 将解析结果中的 litigantslist 赋值给 bean 中的 entities
         */
        ArrayList<ESEntitys> esEntitys = new ArrayList<>();
        JSONArray litigantslist = jo2.getJSONArray("litigantslist");
        if (litigantslist == null) {
            log.error("litigantslist is null");
        }
        /**
         * 对于 entities 中没有的字段或者含义不相同的字段单独赋值
         */
        else {

            HashMap<String, List<String>> hashmap = new HashMap<>();
            for (int i = 0; i < litigantslist.size(); i++) {
                jo3 = JSON.parseObject(litigantslist.getString(i));
                entitys = litigantslist.getObject(i, ESEntitys.class);
                if (jo3 != null && entitys != null) {
                    entitys.organizationname = jo3.getString("companyname");
                    entitys.creditcard = jo3.getString("idcard");
                    entitys.birthdate = jo3.getString("birthday");
                    entitys.vocation = jo3.getString("profession");
                    entitys.creditcode = jo3.getString("orgcard");
                    entitys.nationalDomicile = jo3.getString("ethnic_group");
                    entitys.litigationstatus = jo3.getString("type");


                    if (hashmap.containsKey(entitys.litigationstatus)) {
                        hashmap.get(entitys.litigationstatus).add(entitys.name);
                    } else {
                        ArrayList<String> strings = new ArrayList<>();
                        strings.add(entitys.name);
                        hashmap.put(entitys.litigationstatus, strings);
                    }



                    /**
                     * litigantslist->nametype 为 1 是原告 2是 被告;
                     * 例如type为16的时候 nametype 为1 ，代表此人是原告代理律师。
                     */
                    // TODO: 2021/12/21 代理人赋值
                    entitys.entitie_agents = null;

                    // TODO: 2021/12/21 被代理人赋值
                    entitys.principalname = null;

                    if ("8".equals(entitys.litigationstatus) || "16".equals(entitys.litigationstatus)) {
                        String nametype = jo3.getString("nametype");
                        List<String> strings = hashmap.get(nametype);

                        StringBuilder builder = new StringBuilder();

                        for (int i1 = 0; i1 < strings.size(); i1++) {
                            builder.append(strings.get(i1));

                            if (i1 != strings.size() - 1) {
                                builder.append(',');
                            }
                        }
                        entitys.principalname = builder.toString();
                    }

                    for (ESEntitys esEntity : esEntitys) {
//                        if (esEntity.name);
                    }

                    esEntitys.add(entitys);
                } else if (jo3 == null) {
                    log.error("jo3 is null");
                } else if (entitys == null) {
                    log.error("entitys is null");
                }
            }
        }
        ...
        /**
         * 从解析结果的 judgememberslist 中取出相应的值给变量赋值
         */
        judgememberslist = jo2.getJSONArray("judgememberslist");
        boolean flag = false;
        if (judgememberslist == null) {
            log.error("judgememberslist is null");
        } else {
            for (int i = 0; i < judgememberslist.size(); i++) {
                object = judgememberslist.getObject(i, JudgeMember.class);
                //32 为审判员, 33 为书记员, 34 为陪审员
                ...
                } else if (34 == object.getType()) {
                    if (esDataBean.juror == null) {
                        esDataBean.juror = object.getName();
                    } else {
                        esDataBean.juror += "," + object.getName();
                    }
                } else if (32 == object.getType()) {
                    if (esDataBean.judge == null) {
                        esDataBean.judge = object.getName();
                    } else {
                        esDataBean.judge += "," + object.getName();
                    }
                }

                //给主审法官赋值, 赋值规则为 有审判长 为审判长， 没有审判长，所有的审判员都是主审法官。
                String chiefjudge = esDataBean.chiefjudge;
                int type1 = object.getType();
                if (chiefjudge == null) {
                    if (type1 == 31) {
                        chiefjudge = object.getName();
                        flag = true;
                    } else if (type1 == 32) {
                        chiefjudge = object.getName();
                    }
                ...
                esDataBean.chiefjudge = chiefjudge;
            }
        }

        /**
         * 从 paragraphs 分段信息中取出相应字段信息并赋值
         */
        paragraphs = esDataBean.paragraphs;
        ...
        esDataBean.before_instance = stringStringHashMap.getOrDefault("前审经过", null);
        esDataBean.appellant_opinion = stringStringHashMap.getOrDefault("诉称", null);
        esDataBean.defender_opinion = stringStringHashMap.getOrDefault("辩称", null);
        esDataBean.third_opinion = stringStringHashMap.getOrDefault("第三人称", null);
        esDataBean.court_ascertained = stringStringHashMap.getOrDefault("本案查明", null);
...
//            System.out.println(jo2);
        /**
         * 将 wsBean 和 esDataBean 组合成的新的 bean 转成 jsonString
         */
